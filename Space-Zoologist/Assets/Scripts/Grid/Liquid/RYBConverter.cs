...
public class RYBConverter : MonoBehaviour
{
    ...
    public static Color ToRYBColor(float[] attributes, float[,] interpolationArray = null) // liquidComposition takes 0 - 1, Converts from RYB to RGB color space
    {
        ...
        for (int i = 0; i < 3; i++) // Trilinear interpolation https://en.wikipedia.org/wiki/Trilinear_interpolation
        {
            ...
            {
                cxy[j] = PerformInterpolation(attributes[2], interpolationArray[j, i], interpolationArray[j + 4, i]);
            }
            float c0 = PerformInterpolation(attributes[1], cxy[0], cxy[1]); // 2nd dimention
            float c1 = PerformInterpolation(attributes[1], cxy[2], cxy[3]);
            RGB[i] = PerformInterpolation(attributes[0], c0, c1); // 3rd dimention
        }
