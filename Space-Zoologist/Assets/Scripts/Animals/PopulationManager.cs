...
public class PopulationManager : MonoBehaviour
{
    ...
    public void CreatePopulation(AnimalSpecies species, Vector2Int origin)
    {
        GameObject gameObject = new GameObject();
        gameObject.transform.parent = this.transform;
        gameObject.name = species.SpeciesName;
        gameObject.AddComponent<Population>();
        gameObject.GetComponent<Population>().Initialize(species, origin);
        needSystemManager.RegisterPopulationNeeds(gameObject.GetComponent<Population>());
    }
}
