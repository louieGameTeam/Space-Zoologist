...
public class SpriteFollowCursor : MonoBehaviour
{
    ...
    void Update()
    {
        if (this.Following)
        {
            this.gameObject.transform.position = new Vector3(Input.mousePosition.x, Input.mousePosition.y, this.transform.position.z);// Vector2.Lerp(this.gameObject.transform.position, Input.mousePosition, this.InterpolationValue);
        }
    }
