...
public class QuizConversation : MonoBehaviour
{
    ...
    public (NPCConversation conversation, QuizInstance quiz) Create()
    {
        ...
        NPCConversation conversation = GetComponent<NPCConversation>();
        if (conversation)
        {
#if UNITY_EDITOR 
        ...
        EditableSpeechNode endOfQuiz = CreateSpeechNode(conversation, editableConversation, endOfQuizText, 0, quizTemplate.Questions.Length * 300, false, null);
        nodes.Add(endOfQuiz);
    ...
    private EditableSpeechNode CreateSpeechNode(NPCConversation conversation, EditableConversation editableConversation, string text, float xPos, float yPos, bool isRoot, UnityAction callback)
    {
        ...
        // Setup the node event. We have to do this once at the beginning to initialize the 
